...
class UnitContainer extends ScrollPane {
	...
	public UnitContainer(Consumer<EntryTreeItem> onExpanded) {
	...
	public void updateTitle(EntryTreeItem item) {
	...
	public void setItem(EntryTreeItem item) {
		...
		first().setItem(item);
		int index = 1;
		for (TreeItem<String> ti : item.getChildren()) ((UnitEditor) list.get(index++)).setItem((EntryTreeItem)ti); 
